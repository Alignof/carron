...
pub fn exec(inst: &Instruction, cpu: &mut CPU) -> Result<(), (Option<u32>, TrapCause, String)> {
    ...
    match inst.opc {
        ...
        OpecodeKind::OP_C_ADDI4SPN => {
            cpu.regs.write(inst.rd, cpu.regs.read(Some(REG_SP)) + inst.imm.unwrap() as u32);
        },
        OpecodeKind::OP_C_ADDI => {
            cpu.regs.write(inst.rd, cpu.regs.read(inst.rd) + inst.imm.unwrap() as u32);
        },
        OpecodeKind::OP_C_ADDI16SP => {
            cpu.regs.write(Some(REG_SP), cpu.regs.read(Some(REG_SP)) + inst.imm.unwrap() as u32);
        },
...
mod exe_16 {
    ...
    #[test]
    fn c_extension_test() {
        let dummy_elf = elfload::ElfLoader::try_new("./HelloWorld")
