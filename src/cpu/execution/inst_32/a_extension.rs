...
pub fn exec(inst: &Instruction, cpu: &mut CPU) -> Result<(), (Option<u32>, TrapCause, String)> {
    match inst.opc {
        OpecodeKind::OP_LR_W => {
            let load_addr = cpu.trans_addr(TransFor::StoreAMO, TransAlign::Size32,  cpu.regs.read(inst.rs1))?;
            let _rl = inst.imm.unwrap() & 0x1;
            let _aq = inst.imm.unwrap() >> 1 & 0x1;
            cpu.regs.write(inst.rd, cpu.bus.load32(load_addr)?);
            cpu.reservation_set.insert((load_addr as usize, cpu.regs.read(inst.rd)));
		},
        OpecodeKind::OP_SC_W => {
            let store_addr = cpu.trans_addr(TransFor::StoreAMO, TransAlign::Size32,  cpu.regs.read(inst.rs1))?;
