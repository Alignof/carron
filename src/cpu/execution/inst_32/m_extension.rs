...
pub fn exec(inst: &Instruction, cpu: &mut CPU) -> Result<(), (Option<u32>, TrapCause, String)> {
    ...
    match inst.opc {
        ...
        OpecodeKind::OP_MULH => {
            cpu.regs.write(inst.rd, ((rs1 as i32 as i64 * rs2 as i32 as i64) >> 32) as u32);
        },
        OpecodeKind::OP_MULHSU => {
            cpu.regs.write(inst.rd, ((rs1 as i32 as i64 * rs2 as u32 as u64 as i64) >> 32) as u32);
        },
        OpecodeKind::OP_MULHU => {
            cpu.regs.write(inst.rd, ((rs1 as u32 as u64 * rs2 as u32 as u64) >> 32) as u32);
        },
        OpecodeKind::OP_DIV => {
            if rs2 == 0 {
                cpu.regs.write(inst.rd, u32::MAX); // -1
            } else {
                cpu.regs.write(inst.rd, (rs1 as i32 as i64 / rs2 as i32 as i64)  as u32);
            }
        },
        OpecodeKind::OP_DIVU => {
            if rs2 == 0 {
                ...
                cpu.regs.write(inst.rd, (rs1 as u32 as u64 / rs2 as u32 as u64) as u32);
            }
        },
        OpecodeKind::OP_REM => {
            if rs2 == 0 {
