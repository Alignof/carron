...
pub fn exec(inst: &Instruction, cpu: &mut CPU) -> Result<(), (Option<u32>, TrapCause, String)> {
    ...
    match inst.opc {
        ...
        OpecodeKind::OP_AUIPC => {
            cpu.regs.write(inst.rd, (cpu.pc + inst.imm.unwrap() as u32) as u32);
        },
        OpecodeKind::OP_JAL => {
        ...
        OpecodeKind::OP_ADDI => {
            cpu.regs.write(inst.rd, (cpu.regs.read(inst.rs1) as i32 + inst.imm.unwrap()) as u32);
        },
        OpecodeKind::OP_SLTI => {
        ...
        OpecodeKind::OP_XORI => {
            cpu.regs.write(inst.rd, cpu.regs.read(inst.rs1) ^ inst.imm.unwrap() as u32);
        },
        OpecodeKind::OP_ORI => {
            cpu.regs.write(inst.rd, cpu.regs.read(inst.rs1) | inst.imm.unwrap() as u32);
        },
